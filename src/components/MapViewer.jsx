...
// İkon düzeltmeleri (Kırmızı ve Mavi Pinler)
...
export default function MapViewer() {
    ...
    const unsub = onSnapshot(doc(db, "users", targetUid), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.lastLocation) {
            setTargetLocation([data.lastLocation.latitude, data.lastLocation.longitude]);
            if(data.lastLocation.timestamp) {
                setLastSeen(new Date(data.lastLocation.timestamp.seconds * 1000).toLocaleTimeString());
            }
        }
      }
    });
  ...
  // 3. Kendi Konumumu Al (Mesafe için)
  useEffect(() => {
  ...
  useEffect(() => {
    if (myLocation && targetLocation) {
      ...
      setDistance(dist.toFixed(2)); // km veya m cinsinden
    }
  }, [myLocation, targetLocation]);

  // Matematiksel Mesafe Formülü (Haversine)
  ...
  const handleRing = async () => {
    ...
    if (!confirm("⚠️ DİKKAT: Cihazda yüksek sesli ALARM çalacak. Bu özellik sadece kayıp cihazlar içindir. Onaylıyor musun?")) return;
    ...
    <div style={{ height: "100vh", width: "100vw", position: "relative", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
                <h3 style={{ margin: 0, fontSize: '16px', color: '#333', fontWeight: 'bold' }}>
                   {queryUid ? "Çocuk / Aile Takip" : "Cihazımı Bul"}
                </h3>
                <p style={{ margin: '5px 0 0 0', fontSize: '12px', color: '#666' }}>
                   🕒 Son Sinyal: {targetLocation ? lastSeen : "Aranıyor..."}
                </p>
                {distance && (
      ...
      {/* Alt Panel: Siren (Kayıp Cihaz İçin) */}
      <div style={{
